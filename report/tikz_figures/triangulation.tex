\begin{tikzpicture}[scale=0.75,cap=round]
    ...
    % point on object
    \draw [fill, red] (-6.5,9) circle [radius=0.05];
    \draw node [above right] at (-6.5,\objectdepth) {$p$};

    % left camera
    \draw [fill, blue] (-12,0) circle [radius=0.05];
    ...
    \draw[fill, orange] (pixell) circle [radius=0.05];
    \coordinate (pixelr) at (intersection of ipr1--ipr2 and rayr1--rayr2);
    \draw[fill, orange] (pixelr) circle [radius=0.05];
